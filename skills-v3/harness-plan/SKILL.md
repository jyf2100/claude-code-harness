...
| 用户输入 | 子命令 | 动作 |
|------------|------------|------|
| "创建计划" / "create a plan" | `create` | 对话式访谈 → Plans.md 生成 |
| "添加任务" / "add a task" | `add` | 向 Plans.md 添加新任务 |
| "标记完成" / "mark complete" | `update` | 将任务标记更改为 cc:完了 |
| "今在哪里" / "check progress" | `sync` | 照合实现与 Plans.md・同步 |
| `harness-sync` | `sync` | 进度确认（与独立 sync surface 等同） |
...
**回顾**（默认 ON）:
如有 1 件以上 `cc:完了` 任务则自动执行回顾。
分析估算精度、阻塞原因模式、范围变动，记录学习内容。
...
**DoD（Definition of Done）**: 用 1 行描述可验证的完成条件。禁止"感觉不错""正常运行"。必须能用 Yes/No 判定。

**Depends**: 任务间的依赖关系。`-`（无依赖）、任务编号（`N.1`）、逗号分隔（`N.1, N.2`）、阶段依赖（`Phase N`）。
