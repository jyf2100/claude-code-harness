...
| スキル | トリガーキーワード | 除外 |
|--------|-----------------|------|
| `plan` | create a plan, add tasks, update Plans.md, mark complete, check progress, sync status, where am I, /plan, /sync-status | implementation, code review, release |
| `execute` | implement, execute, /execute, /work, do everything, build features, run tasks, breezing, team run, --codex, --parallel | planning, code review, release, setup |
| `review` | review, code review, plan review, scope analysis, security, performance, quality checks, PRs, diffs, /review | implementation, new features, bug fixes, setup, release |
| `release` | release, version bump, create tag, publish, /release | implementation, code review, planning, setup |
| `setup` | setup, initialization, new project, CI setup, codex CLI setup, harness-mem, agent setup, symlinks, /setup | implementation, code review, release, planning |
