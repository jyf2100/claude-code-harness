...
| 用户输入 | 模式 | 动作 |
|------------|--------|------|
| `harness-work` | **auto** | 根据任务数自动判定（见下） |
| `harness-work all` | **auto** | 以自动模式执行所有未完成任务 |
| `harness-work 3` | solo | 仅执行任务3 |
| `harness-work --parallel 5` | parallel | 5个工作者并行执行（强制） |
| `harness-work --codex` | codex | 委托给 Codex CLI（仅限明确指定） |
| `harness-work --breezing` | breezing | 强制团队执行 |

## Execution Mode Auto Selection（无标志时的自动判定）

当没有明确的模式标志（`--parallel`, `--breezing`, `--codex`）时，
根据目标任务数自动选择最优模式:

| 目标任务数 | 自动选择模式 | 理由 |
|-------------|---------------|------|
| **1 件** | Solo | 开销最小。直接实现最快 |
| **2〜3 件** | Parallel（Task tool） | Worker 分离开始体现优势的阈值 |
...
2. **`--codex` 仅在明确指定时生效**。因为有些环境未安装 Codex CLI，所以不自动选择
...
| `--sequential` | 强制串行执行 | - |
| `--codex` | 委托 Codex CLI 实现（仅限明确指定，不自动选择） | false |
...
> 可以启用 plugin settings 的 `includeGitInstructions: false` 来
> 减少提示词 token。
...
有参数则立即执行（跳过对话）:
- `harness-work all` → 所有任务，自动模式选择
- `harness-work 3-6` → 4件所以自动选择 Breezing

## Effort 级别控制（v2.1.68+, v2.1.72 简化）

Claude Code v2.1.68 中 Opus 4.6 默认为 **medium effort** (`◐`)。
v2.1.72 废弃了 `max` 级别，简化为 3 档 `low(○)/medium(◐)/high(●)`。
可通过 `/effort auto` 重置为默认值。
复杂任务可使用 `ultrathink` 关键字启用 high effort (`●`)。
...
分数 ≥ 3 时，在 Worker spawn prompt 开头添加 `ultrathink`。
breezing 模式也适用相同逻辑（由 harness-work 统一管理）。
...
1. 读取 Plans.md，确定目标任务
   ...
   - **会话中有未记载任务时**: 从最近的会话上下文提取需求，自动以 `cc:TODO` 追加到 Plans.md
     - 提取逻辑: 从用户发言中检测动作动词（"添加〜"、"修正〜"、"实现〜"）
     - 追加时遵循 v2 格式（Task / 内容 / DoD / Depends / Status）
     - 追加后显示"已向 Plans.md 追加以下内容"（5 秒超时提示，默认: 继续）
1.5. **任务背景确认**（30 秒）:
   - 从任务的"内容"和"DoD"推论显示 **目的**（该任务要解决的问题）1 行
   - 用 `git grep` / `Glob` 推论显示 **影响范围**（变更涉及的文件/模块）
   - 推论有把握时: 直接进入实现（不延迟流程）
   - 推论没把握时: 只向用户确认 1 个问题（"这个理解正确吗？"）
2. 将任务更新为 `cc:WIP`
3. **TDD 阶段**（无 `[skip:tdd]` 且存在测试框架时）:
   a. 先创建测试文件（Red）
   b. 确认失败
4. 实现代码（Green）（Read/Write/Edit/Bash）
5. 用 `/simplify` 进行 Auto-Refinement（可用 `--no-simplify` 省略）
6. **自动审查阶段**（参见"审查循环"）:
   - 优先用 Codex exec 执行审查 → 回退到内部 Reviewer agent
   - REQUEST_CHANGES 时: 根据指摘修正→再审查（最多 3 次）
   - APPROVE 则进入下一步
7. 用 `git commit` 自动提交（可用 `--no-commit` 省略）
8. 将任务更新为 `cc:完了`（附带 commit hash）
   - 用 `git log --oneline -1` 获取最近的 commit hash（7 字短格式）
   - 将 Plans.md 的 Status 更新为 `cc:完了 [a1b2c3d]` 格式
   - 无 commit 时（`--no-commit` 时）仅 `cc:完了` 不带 hash
9. **丰富完成报告**（参见"完成报告格式"）
10. **失败时的自动重新规划**（仅测试/CI 失败时）:
    - 确认测试执行结果
    - 失败时: 将修正任务方案保存到 state，通过批准命令追加到 Plans.md（参见"失败任务的自动重新工单化"）
    - 成功时: 进入下一任务

### Parallel 模式（2〜3 件时自动选择 / 用 `--parallel N` 强制）

用 N 个工作者并行执行带 `[P]` 标记的任务。
用 `--parallel N` 明确指定时，无论任务数都使用此模式。
写入同一文件产生冲突时用 git worktree 隔离。
...
> **CC v2.1.69+**: nested teammates 被平台方禁止，
> 因此 Worker/Reviewer 提示词中不添加冗余的 nested 防止措辞。
...
对每个任务**逐次**执行以下操作（按依赖顺序）:

> **API 注记**: 以下用 Claude Code 的 API 语法描述。
> Codex 环境中请将 `Agent(...)` → `spawn_agent(...)`, `SendMessage(...)` → `send_input(...)` 进行转换。
...
for task in execution_order:
    ...
    while verdict == "REQUEST_CHANGES" and review_count < 3:
        ...
        # Worker 修正 → amend → 返回更新的 commit hash
...
4. `reject fix <task_id>` 则放弃方案。仅 1 件 pending 时也可用 `yes` / `no` 响应

## 审查循环

实现完成后（步骤 5 之后）自动执行的质量验证阶段。
**所有模式通用**（Solo / Parallel / Breezing）统一适用。
Parallel 模式中各 Worker 作为 step 10（外部审查受理）执行相同循环。
...
```bash
# 任务开始时记录 base ref（在 Step 2 的 cc:WIP 更新前执行）
...
- major（破坏现有功能・规格矛盾・测试不通过）: 1 件即 → REQUEST_CHANGES
- minor（命名・注释・风格）: 不影响 verdict → APPROVE
- recommendation（改进建议）: 不影响 verdict → APPROVE

仅 minor / recommendation 时请务必返回 APPROVE。

## 额外观点
- AI Residuals（mock / dummy / localhost / TODO / test.skip / 硬编码配置等）
- 将 `scripts/review-ai-residuals.sh` 的结果作为依据之一，仅将属于 major 的反映到 verdict
- 单纯的残留候补和临时实现注释请保持在 minor / recommendation
...
if review_count >= MAX_REVIEWS and verdict != "APPROVE":
    → 升级给用户
    → "已修正 3 次但以下 critical/major 指摘仍存在" + 显示指摘列表
    → 等待用户判断（继续 / 中断）
```
...
3. REQUEST_CHANGES → Lead 用 SendMessage 向 Worker 发送修正指示 → Worker 进行 amend
4. 修正后，再审查（最多 3 次）
5. APPROVE → Lead cherry-pick 到 main → 将 Plans.md 更新为 `cc:完了 [{hash}]`
...
2. **有什么变化**: 从任务的"内容"和"DoD"推论 Before/After。重视用户体验的变化
3. **变更文件**: 从 `git diff --name-only HEAD~1` 获取。超过 5 个文件则省略并显示件数
4. **剩余课题**: 列出 Plans.md 中的 `cc:TODO` / `cc:WIP` 任务。明确表示是否已记载于 Plans.md
...
所有任务完成后一并输出。各任务以简化版（做了什么 + commit hash）列出，
最后输出整体摘要（合计变更文件数 + 剩余课题）:
