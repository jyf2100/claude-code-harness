...
# 从 .claude/state/snapshots/progress-*.json 生成最新快照摘要的公共函数
#
...
phase = latest.get("phase", "未知")
ts = latest.get("timestamp", "未知")
...
lines = [
    ...
    f"   进度率 {rate}% / 完成 {done} / WIP {wip} / TODO {todo}",
]
...
if previous:
    ...
    lines.append(
        "   较上次: "
        f"进度率 {rate - prev_rate:+d}%pt / 完成 {done - prev_done:+d} / "
