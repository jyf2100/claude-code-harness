...
if [ "$QUALITY_PRETTIER" = "true" ]; then
  if [ "$QUALITY_MODE" = "run" ]; then
    if run_prettier; then
      append_feedback "🧹 Prettier: 已执行"
    else
      append_feedback "🧹 Prettier: 未执行（未找到 prettier）"
    fi
  else
    append_feedback "🧹 Prettier: 推荐（示例: npx prettier --write \"$FILE_PATH\"）"
  fi
fi
...
if [ "$QUALITY_TSC" = "true" ]; then
  if [ "$QUALITY_MODE" = "run" ]; then
    if run_tsc; then
      append_feedback "🧪 tsc --noEmit: 已执行"
    else
      append_feedback "🧪 tsc --noEmit: 未执行（未检测到 tsconfig/tsc）"
    fi
  else
    append_feedback "🧪 tsc --noEmit: 推荐"
  fi
fi
...
if [ "$QUALITY_CONSOLE_LOG" = "true" ]; then
  if [ -f "$FILE_PATH" ]; then
    ...
    if [ "$CONSOLE_LOG_COUNT" -gt 0 ]; then
      append_feedback "⚠️ 发现 ${CONSOLE_LOG_COUNT} 处 console.log"
    fi
  fi
fi
