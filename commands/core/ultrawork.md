...
しかし ultrawork の自律実行中は、これらの確認で作業が中断されてしまいます。

**ultrawork モード中の動作**:

1. Phase 1 で `.claude/state/ultrawork-active.json` を作成
2. フックがこのファイルを検出し、特定の確認をスキップ
3. 完了/中断時にファイルを削除

**バイパス対象（自動承認）**:
- `rm -rf` - キャッシュクリア等の戦略で使用
- `git push` - 中間チェックポイントで使用
