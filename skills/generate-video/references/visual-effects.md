...
**使用示例**：
```tsx
<GlitchText text="创新功能" fontSize={64} startFrame={0} />
```
...
<div style={{ display: "flex", gap: 40, perspective: 1000 }}>
  ...
  <ParallaxCard delay={70} color="#FF00FF">Intro 演示</ParallaxCard>
  ...
  return (
    <AbsoluteFill style={{ background: "#0A0A0F" }}>
      ...
      >
        ...
        <div style={{ height: 20 }} />
        <GlitchText text="进入自动生成时代" fontSize={64} startFrame={15} />
      </div>
...
| `interpolate` | 用 `extrapolateRight: "clamp"` 防止值失控 |
| `spring` | 用 `config: { damping: 200 }` 变流畅 |
| CSS animations | 禁止使用，用 Remotion 的 `useCurrentFrame()` |
