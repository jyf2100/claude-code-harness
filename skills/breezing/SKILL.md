...
/breezing [range] [--codex-review] [--parallel N]
    │
Phase 0: 範囲確認 → ユーザー承認
Phase 1: Team 初期化 → delegate mode → Teammates spawn
Phase 2: 実装サイクル → Implementer 並列実装 → self-claim → build/test
Phase 3: レビュー/リテイクループ → Reviewer 独立レビュー → APPROVE まで
Phase 4: 完了処理 → commit → cleanup → handoff
```
