...
description: "使用 session-state.sh 执行会话状态转换"
...
./scripts/session-state.sh --state escalated --event escalation.requested \
  --data '{"reason":"构建失败 3 次","retry_count":3}'
```

## 期望的结果

- `.claude/state/session.json` 的 `state`、`updated_at`、`last_event_id`、`event_seq` 被更新
- `.claude/state/session.events.jsonl` 中追加事件
- 非法转换在 stderr 输出错误 + 非零退出

## 错误处理

转换失败时（如非法转换）：
1. 在 stderr 输出当前状态和允许的转换
2. 返回非零退出码
