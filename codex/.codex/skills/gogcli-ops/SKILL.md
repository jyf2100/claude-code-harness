---
name: gogcli-ops
description: "用 gogcli 操作 Google Workspace（Drive/Sheets/Docs/Slides）。当用户要求通过 gogcli 确认/搜索/导出/读取/更新 Google 文件时使用。Trigger when a user asks to check, list, search, export, read, or update Google files via gogcli; when a Google URL/ID needs parsing; when auth/account selection or safe read-only workflows are needed; or when troubleshooting gogcli access/errors. Do NOT load for: general file operations, non-Google cloud storage, or standard shell commands."
