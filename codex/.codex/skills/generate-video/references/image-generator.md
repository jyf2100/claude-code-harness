# Image Generator - Nano Banana Pro 图像自动生成

使用 Nano Banana Pro（Google DeepMind）自动生成视频场景用的高品质图像。
...
| `gemini-2.5-flash-image` | 快速・低成本 | 1024px |
...
> **注意**: Gemini API 使用 `x-goog-api-key` 头部。虽然也支持查询参数方式（`?key=...`），但推荐使用头部方式。
...
> **注**: `responseModalities` 可以指定 `["TEXT", "IMAGE"]` 或 `["IMAGE"]`。本流程为质量判定也获取文本说明，因此指定两者。
...
| `2K` | 2048×2048 | 标准品质 | ~$0.06/张 |
| `4K` | 4096×4096 | 高品质、专业 | ~$0.12/张 |
...
|-----------|---------|---------------|-----------|
| 90秒预告片 | 5 | 2-3 | $0.48-$0.72 |
| 3分钟演示 | 8 | 3-4 | $0.72-$0.96 |
| 5分钟架构 | 12 | 4-6 | $0.96-$1.44 |
