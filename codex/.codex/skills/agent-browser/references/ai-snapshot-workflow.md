...
利用 agent-browser 的 `snapshot` 命令为 AI 代理设计的工作流。
...
2. **把握上下文**: 元素的角色（button、input、link）明确
3. **确定性操作**: 通过 `@e1` 等引用可靠地操作
...
**选项说明**:
- `-i, --interactive`: 仅显示交互元素（按钮、链接、输入字段等）
- `-c, --compact`: 移除空的结构元素，更紧凑
