---
name: harness-setup
description: "Harness v3 统一设置技能。负责项目初始化、工具设置、2 代理配置、内存设置、公开 skill mirror 同步。以下触发: 设置、初始化、新项目、CI 设置、codex CLI 设置、harness-mem、代理设置、symlink、mirror、harness-setup。不用于实现、审查、发布、规划。"
...
Windows 的 `core.symlinks=false` 会导致 repository symlink 变成普通文件，`harness-*` skill 可能不会出现在命令列表中。公开 bundle 作为实际目录 mirror 同步。
...
v2.1.71 修复了更新时的 merge conflict，可以实现稳定的更新。

### 其他改进

- MCP 服务器去重：自动防止同一 MCP 服务器的重复注册
