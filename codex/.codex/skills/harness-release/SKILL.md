...
description: "Harness v3 统一发布技能。自动化 CHANGELOG、版本升级、标签、GitHub Release。以下触发: 发布、版本升级、创建标签、公开、/harness-release。不用于实现、代码审查、规划、设置。"
...
```bash
/release          # 交互式（确认版本类型）
/release patch    # 补丁版本升级（错误修正）
/release minor    # 次版本升级（新功能）
/release major    # 主版本升级（破坏性变更）
/release --dry-run  # 仅预览（不执行）
/release --announce # 同时发送到 Slack 等
```

## Release-only policy

- 普通 PR: 不修改 `VERSION` / `.claude-plugin/plugin.json` / versioned `CHANGELOG.md` entry
- 普通 PR 的变更记录: 追加到 `CHANGELOG.md` 的 `[Unreleased]`
- 仅在 `/release` 执行时统一更新 version bump、versioned CHANGELOG entry、tag / GitHub Release

## 分支策略

- **单人开发**: 允许直接 push 到 main（CI 作为质量关卡）
- **协作开发**: 必须通过 PR 合并
...
遵循语义版本控制（SemVer）：
- `patch`: x.y.Z → x.y.(Z+1)（错误修正）
- `minor`: x.Y.z → x.(Y+1).0（新功能、向后兼容）
- `major`: X.y.z → (X+1).0.0（破坏性变更）

### Step 3: 更新 CHANGELOG

Release entry 以将普通 PR 积累的 `[Unreleased]` 变更整理到版本化章节的方式编写。

用**详细 Before/After 格式**（日语）描述。
各功能用编号的独立章节，说明"至今"和"今后"并附具体示例。
