---
name: init-memory-ssot
description: "プロジェクトのSSOTメモリ（decisions/patterns）と任意のsession-logを初期化する。初回セットアップ時や、.claude/memory が未整備のプロジェクトで使用します。"
allowed-tools: ["Read", "Write"]
---

# Init Memory SSOT

初始化 `.claude/memory/` 下的 **SSOT**。

- `decisions.md`（重要决策的 SSOT）
- `patterns.md`（可复用方案的 SSOT）
- `session-log.md`（会话日志。推荐本地管理）

详细方针: `docs/MEMORY_POLICY.md`

---

## 执行步骤

### Step 1: 确认现有文件

- `.claude/memory/decisions.md`
- `.claude/memory/patterns.md`
- `.claude/memory/session-log.md`

已存在的文件**不覆盖**。

### Step 2: 从模板初始化（仅在不存在时）

模板:

- `templates/memory/decisions.md.template`
- `templates/memory/patterns.md.template`
- `templates/memory/session-log.md.template`

将 `{{DATE}}` 替换为当天日期（例: `2025-12-13`）生成。

### Step 3: 完成报告

- 已创建的文件列表
- Git 方针（`decisions/patterns` 推荐共享，`session-log/.claude/state` 推荐本地管理）


