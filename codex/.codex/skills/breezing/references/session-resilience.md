...
> 互換性メモ（2026-02-18）:
> Codex 実行時は `impl_mode: "codex"` が既定。`--claude` 指定時のみ `impl_mode: "claude"` を使う。
...
| `impl_mode` | 実装モード (`"codex"` or `"claude"`)。Compaction 復元キー | Step 0 で即時書き込み |
...
| なし | `impl_mode` 追加 | Compaction 復元キー (`"codex"` or `"claude"`) |
...
impl_mode を確認:
  ...
  ├── "claude" → Claude 委譲モード → Team 再構築 (task-worker spawn)
  └── なし or 未設定 → Codex 既定として扱う（後方互換）
    ↓
現在のスキルとモード不一致チェック:
  ├── /breezing --claude 実行中に impl_mode="codex" → ユーザーに確認
  └── /breezing 実行中に impl_mode="claude" → ユーザーに確認
    ↓
...
```jsonc
{
  "impl_mode": "codex",  // 通常 breezing（Codex 既定）
  // ... 既存フィールド
}
```
