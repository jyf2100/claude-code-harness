...
/breezing --claude all                        # Claude で全タスク完走
/breezing --claude --parallel 2 ログイン機能   # Claude 2並列で範囲指定
/breezing --codex all                         # 互換エイリアス（既定と同じ）
/breezing --parallel 2 ログイン機能            # Codex 2並列で範囲指定
...
| `all` | 全未完了タスクを対象 | - |
| `N` or `N-M` | タスク番号/範囲指定 | - |
| `--claude` | Claude CLI で実装委譲 | false |
| `--codex` | 互換エイリアス（既定と同じ Codex 実装） | true (既定) |
...
Codex 実行時は **Codex が既定実装主体**。`--claude` 指定時のみ Claude に委譲:

| 項目 | デフォルト (Codex) | --claude |
|------|-----------|---------|
| Implementer | codex-implementer (Codex) | task-worker (Claude) |
| 実装の仕組み | Codex が直接コーディング | Claude CLI 経由で委託 |
| 品質保証 | セルフレビュー 4 観点 | AGENTS_SUMMARY + Quality Gates |
| ファイル分離 | owns: アノテーション | Lead 判断（worktree or owns:） |
| コスト特性 | Codex トークン消費 | Claude API + Codex レビュー |

互換性メモ: `--codex` は legacy alias として受理し、既定動作（Codex 実装）から変更しない。
...
3. (--claude) Claude CLI が利用可能（`which claude`）

## Execution Flow Summary

```
/breezing [scope] [--claude] [--codex] [--parallel N]
    │
Phase A: Pre-delegate（ユーザーのパーミッションモード維持）
  Step 0: breezing-active.json に impl_mode を即時書き込み
          --claude あり → impl_mode: "claude"
          --claude なし → impl_mode: "codex"
  ...
  Step 3: Team 初期化 → TaskCreate → Implementer N 個を同時 spawn:
          ...
          impl_mode: "claude" → subagent_type: task-worker
...
Phase B: Delegate（Lead は調整専念）
  ...
  │        (既定: codex-implementer が Codex 直接実装)
  │        (--claude: task-worker が Claude CLI 経由で実装)
...
- `impl_mode: "claude"` のときに `codex-implementer` を spawn することは禁止（必ず `task-worker`）
...
5. (--claude) 全タスクの AGENTS_SUMMARY 検証通過
...
Tip: --claude を付けると Claude に実装を委託できます
```
