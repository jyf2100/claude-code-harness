...
/work all                # 全タスクを即実行
/work 3-6                # タスク3〜6を即実行
/work --claude           # Claude に実装委譲（スコープを聞く）
/work --claude all       # Claude で全タスク即実行
...
Codex 実行時は **Codex が既定の実装主体**。`--claude` 指定時のみ Claude に委譲:

| 項目 | デフォルト (Codex) | --claude |
|------|-----------|---------|
| 実装主体 | Codex (直接実装) | Claude CLI |
