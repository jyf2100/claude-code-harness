...
|------|------|
| **Phase A** | 準備・タスク分解 |
| **Phase B** | delegate + review — Worker spawn / レビュー実行 / SendMessage / cherry-pick |
| **Phase C** | リッチ完了報告・Plans.md 最終更新 |
| **禁止** | Phase B 中の直接 Write/Edit（実装は Worker に委譲）。ただし Bash はレビュー（codex exec）と cherry-pick で使用可 |
...
| **禁止** | Task（再帰防止） |
| **責務** | 実装 → セルフレビュー → CI検証 → worktree 内 commit（Breezing 時は main に反映しない） |
...
```
Phase A: Lead がタスクを分解・依存グラフ解析・effort スコアリング
    ↓
Phase B: タスクごとに逐次実行（依存順）
    ↓
    B-1. Worker spawn (mode: breezing, isolation: worktree)
         Worker: 実装 → セルフレビュー → worktree 内 commit → Lead に結果返却
    ↓
    B-2. Lead がレビュー実行
         Codex exec（優先、--sandbox read-only）
    ...
    B-4. APPROVE → Lead が main に cherry-pick
         ...
         Plans.md: cc:完了 [{hash}]
    ↓
Phase C: Lead がリッチ完了報告を出力・Plans.md 最終確認
```

### SendMessage パターン（修正ループ）

Lead が Worker に修正を指示する際の構文:
...
Worker 側の受信処理:
1. SendMessage を受信 → 指摘内容を解析
2. 該当箇所を修正
3. `git commit --amend` で worktree 内の commit を更新
4. 更新された commit hash を Lead に返却

### cherry-pick パターン（APPROVE 後）

Lead が Worker の worktree commit を main に取り込む:

```bash
# worktree 内の commit を main に cherry-pick
git cherry-pick --no-commit {worktree_commit_hash}
# Lead が commit message を制御
git commit -m "feat: {task_description}"
```

> **注意**: `--no-commit` で staged 状態にしてから commit することで、
> Lead が commit message を統一フォーマットに制御できる。
...
Codex CLI 環境では Claude Code の Agent/SendMessage API が利用できない。
native subagent API（`spawn_agent`, `send_input` 等）で代替する。
...
| `SendMessage(to, message)` | `send_input(agent_id, message)`。agent が閉じている場合は先に `resume_agent(agent_id)` | 修正ループで使用 |
| `bypassPermissions` | `--full-auto` | codex exec は非対話 |
| Task ツール | Plans.md 直接編集 | |
| PreToolUse hooks | config.toml sandbox | |

> **重要**: harness-work / breezing の Phase B フロー（Worker spawn → レビュー → cherry-pick）は
> Claude Code の `Agent` + `SendMessage` 構文で記述しています。
> Codex 環境では上記マッピング表に従って読み替えてください。
