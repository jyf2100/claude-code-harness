...
description-ja: "提供 Cursor ↔ Claude Code 双代理工作流指导。Use when user asks about workflow, collaboration, or process. Do NOT load for: implementation work, workflow setup, or executing handoffs."
allowed-tools: ["Read"]
...
Cursor ↔ Claude Code 双代理工作流的指导技能。
...
| **Claude Code** | Worker（工作者） | 实现、测试、CI修复、staging部署 |
...
A: 即使一个人工作，也建议使用 Plans.md 管理任务。
生产部署请谨慎手动进行。

### Q: 如果任务不明确怎么办？

A: 请请求 Cursor 确认，或用 `/sync-status` 整理现状。

### Q: CI 多次失败怎么办？

A: 超过3次不要自动修复，请升级给 Cursor。
