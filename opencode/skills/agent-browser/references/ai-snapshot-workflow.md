...
活用 agent-browser 的 `snapshot` 命令的 AI 代理工作流。

---

## 概要

`snapshot` 命令获取页面的可访问性树，为每个元素赋予引用 ID（`@e1`、`@e2` 等）。这样可以：

1. **无需 CSS 选择器**: 不依赖动态 ID 或类名
2. **把握上下文**: 元素角色（button、input、link）明确
3. **确定性操作**: 通过 `@e1` 等引用可以可靠操作
