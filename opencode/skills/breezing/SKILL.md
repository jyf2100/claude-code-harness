...
Phase A: Pre-delegate（ユーザーのパーミッションモード維持）
  スコープ確認 → ユーザー承認 → breezing-active.json Write → Team 初期化 → Teammates spawn
...
**絶対禁止**: `impl_mode: "codex"` がある限り、Lead が Write/Edit でソースコードを直接書くことは禁止。
