...
对单独场景 JSON 进行 `scene.schema.json` 验证。
...
对场景 JSON 进行 `scenario.schema.json` 验证。
同时也执行语义检查：
- Section ID 的唯一性
- Section 顺序的正确性
- Duration 的合理性
...
Critical 错误会停止验证，Warning 会输出日志并继续。
