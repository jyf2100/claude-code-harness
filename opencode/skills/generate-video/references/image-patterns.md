...
定义针对视频场景优化的图像模式。每种模式都针对特定目的优化，与 AI 图像生成提示词模板联动。

### 模式列表

| 模式 | 用途 | 最佳场景 | 提示词模板 |
|---------|------|-----------|---------------------|
| **comparison** | Before/After、好例子/坏例子对比 | 问题提出、改善效果展示 | `templates/image-prompts/comparison.txt` |
| **concept** | 抽象概念、层次结构、关系可视化 | 架构讲解、概念说明 | `templates/image-prompts/concept.txt` |
...
视觉对比 Before/After、好例子/坏例子等两种状态或选择。

### 使用场景

| 场景 | 示例 |
|--------|-----|
| **问题提出** | 现有工具的复杂 vs 本产品的简单 |
...
| 布局 | 用途 | 视觉印象 |
|-----------|------|------------|
| **hierarchy** | 层次结构（组织图、依赖关系） | 从上到下的树 |
| **radial** | 从中心放射（生态系统） | 中央主要要素、周围关联要素 |
| **grid** | 并列配置（类别分类） | 矩阵配置 |
| **flow** | 处理流程（管道） | 从左到右的流向 |
| **circular** | 循环流程（生命周期） | 圆环状 |
...
| 场景类型 | 第 1 推荐 | 第 2 推荐 | 用途 |
|------------|---------|---------|------|
| **Hook** | highlight | comparison | 强烈第一印象 |
| **Problem** | comparison | concept | 明确现状问题 |
| **Solution** | concept | flow | 解决方案的机制 |
| **Demo** | flow | comparison | 可视化步骤 |
...
| 15-55秒 | Solution | **flow** | 设置 → 执行 → 完成的 3 步 |
...
- **必须**: `type`, `topic` 字段必须
- **oneOf**: 根据模式需要专用字段（例: type="comparison" 则 comparison 字段必须）
- **验证**: 用 `scripts/validate-visual-pattern.js` 验证
...
- **自动判定**: 用 `references/image-quality-check.md` 进行质量评估
- **重试**: 不合格时最多重新生成 3 次
- **确定性**: 保存 seed 值确保可重现
...
- [image-generator.md](./image-generator.md) - AI 图像生成实现
